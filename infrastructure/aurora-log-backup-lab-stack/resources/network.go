...
func CreateNetworkResources(ctx *pulumi.Context, cfg *config.Config) (*NetworkResources, error) {
	...
	vpc, err := ec2.NewVpc(ctx, "aurora-vpc", &ec2.VpcArgs{
		...
		Tags:               utils.CreateResourceTags(ctx, cfg.Tags, "aurora-vpc"),
	})
	if err != nil {
		return nil, err
	}
	...
	publicSubnet, err := ec2.NewSubnet(ctx, "public-subnet", &ec2.SubnetArgs{
		...
		Tags:             utils.CreateResourceTags(ctx, cfg.Tags, "aurora-public-subnet"),
	})
	if err != nil {
		return nil, err
	}
	...
	privateSubnet1, err := ec2.NewSubnet(ctx, "private-subnet-1", &ec2.SubnetArgs{
		...
		Tags:             utils.CreateResourceTags(ctx, cfg.Tags, "aurora-private-subnet-1"),
	})
	if err != nil {
		return nil, err
	}
	...
	privateSubnet2, err := ec2.NewSubnet(ctx, "private-subnet-2", &ec2.SubnetArgs{
		...
		Tags:             utils.CreateResourceTags(ctx, cfg.Tags, "aurora-private-subnet-2"),
	})
	if err != nil {
		return nil, err
	}
	...
	igw, err := ec2.NewInternetGateway(ctx, "aurora-igw", &ec2.InternetGatewayArgs{
		VpcId: vpc.ID(),
		Tags:  utils.CreateResourceTags(ctx, cfg.Tags, "aurora-igw"),
	})
	if err != nil {
		return nil, err
	}
	...
	s3VpcEndpoint, err := ec2.NewVpcEndpoint(ctx, "s3-vpc-endpoint", &ec2.VpcEndpointArgs{
		...
		Tags:            utils.CreateResourceTags(ctx, cfg.Tags, "aurora-s3-vpc-endpoint"),
	})
	if err != nil {
		return nil, err
	}
	...
	dynamoDBVpcEndpoint, err := ec2.NewVpcEndpoint(ctx, "dynamodb-vpc-endpoint", &ec2.VpcEndpointArgs{
		...
		VpcEndpointType: pulumi.String("Gateway"),
		RouteTableIds:   pulumi.StringArray{}, // We'll associate it with private route table later
		Tags:            utils.CreateResourceTags(ctx, cfg.Tags, "aurora-dynamodb-vpc-endpoint"),
	})
	if err != nil {
		return nil, err
	}
	...
	vpcEndpointSG, err := ec2.NewSecurityGroup(ctx, "vpc-endpoint-sg", &ec2.SecurityGroupArgs{
		...
		Tags: utils.CreateResourceTags(ctx, cfg.Tags, "vpc-endpoint-sg"),
	})
	if err != nil {
		return nil, err
	}
	...
	rdsVpcEndpoint, err := ec2.NewVpcEndpoint(ctx, "rds-vpc-endpoint", &ec2.VpcEndpointArgs{
		...
		Tags:              utils.CreateResourceTags(ctx, cfg.Tags, "aurora-rds-vpc-endpoint"),
	})
	if err != nil {
		return nil, err
	}
	...
	sqsVpcEndpoint, err := ec2.NewVpcEndpoint(ctx, "sqs-vpc-endpoint", &ec2.VpcEndpointArgs{
		...
		SecurityGroupIds:  pulumi.StringArray{vpcEndpointSG.ID()},
		PrivateDnsEnabled: pulumi.Bool(true),
		Tags:              utils.CreateResourceTags(ctx, cfg.Tags, "aurora-sqs-vpc-endpoint"),
	})
	if err != nil {
		return nil, err
	}
	...
	publicRouteTable, err := ec2.NewRouteTable(ctx, "public-rt", &ec2.RouteTableArgs{
		...
		Tags: utils.CreateResourceTags(ctx, cfg.Tags, "aurora-public-rt"),
	})
	if err != nil {
		return nil, err
	}
	...
	privateRouteTable, err := ec2.NewRouteTable(ctx, "private-rt", &ec2.RouteTableArgs{
		VpcId: vpc.ID(),
		Tags:  utils.CreateResourceTags(ctx, cfg.Tags, "aurora-private-rt"),
	})
	if err != nil {
		return nil, err
	}
